...
import '../../styles/coordinator-responsive.css';
import UnitView from '../../components/coordinator/UnitView';
import ReportManagement from '../../components/coordinator/ReportManagement';
import Reports from '../../components/coordinator/Reports';
import UserManagement from '../../components/coordinator/UserManagement';
import ComplianceView from '../../components/coordinator/ComplianceView';
import { Notifications, HistoryView, SettingsView } from '../../components/coordinator/SharedViews';
...
function SidebarComponent({ activeView, onViewChange, onLogout, role, notifCount = 3, pendingCount = 0, userName = 'Coordinador' }) {
  ...
  const principalItems = role === 'coordinator' ? [
    { key: 'unit',              label: 'Unidad', icon: '🏠' },
    { key: 'report-management', label: 'Gestión de informes', badge: notifCount, icon: '📋' },
    { key: 'reports',           label: 'Reportes', icon: '📊' },
    { key: 'compliance',        label: 'Cumplimiento', icon: '🎯' },
    { key: 'user-management',   label: 'Usuarios', badge: pendingCount, icon: '👥' },
  ...
  const renderItem = (item) => {
    ...
    return (
      ...
      }} title={!expanded ? item.label : undefined} style={{
  ...
  return (
    ...
    {expanded && (
      <div
        className="coord-sidebar-backdrop"
    ...
    <aside className="coord-sidebar" data-expanded={expanded} style={{
      width: expanded ? 220 : 72, minWidth: expanded ? 220 : 72,
      borderRight: '1px solid #E8ECF0', background: '#fff',
      display: 'flex', flexDirection: 'column', height: '100vh',
      position: 'sticky', top: 0, transition: 'width .18s ease, min-width .18s ease',
      overflow: 'hidden',
    }}>
      {/* Logo SENA + botón para abrir/cerrar */}
      <div style={{ padding: expanded ? '18px 16px 14px' : '18px 0 14px', borderBottom: '1px solid #F0F2F5' }}>
          ...
          {expanded ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #16a34a, #15803d)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 11, letterSpacing: 0.5, flexShrink: 0 }}>SENA</div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#111827', lineHeight: 1.2, whiteSpace: 'nowrap' }}>Portal Coordinador</div>
                  <div style={{ fontSize: 11, color: '#9CA3AF', whiteSpace: 'nowrap' }}>Sistema de Gestión</div>
                </div>
              </div>
              <button onClick={() => setExpanded(false)} title="Cerrar menú" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#9CA3AF', padding: 4, flexShrink: 0, display: 'flex' }}
            ...
            <button onClick={() => setExpanded(true)} title="Abrir menú" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#16a34a', padding: 4, display: 'flex' }}>
      ...
      {/* Usuario logueado */}
      <div style={{ padding: expanded ? '12px 16px' : '12px 0', borderBottom: '1px solid #F0F2F5', display: 'flex', alignItems: 'center', gap: 10, justifyContent: expanded ? 'flex-start' : 'center' }}>
        <div style={{
      ...
      <div style={{ padding: expanded ? '12px 10px' : '12px 8px', borderTop: '1px solid #F0F2F5' }}>
        <button onClick={onLogout} title={!expanded ? 'Cerrar Sesión' : undefined} style={{ width: '100%', padding: expanded ? '9px 12px' : '9px 0', border: 'none', borderRadius: 10, background: 'transparent', color: '#ef4444', cursor: 'pointer', textAlign: 'left', fontSize: 13.5, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 10, justifyContent: expanded ? 'flex-start' : 'center', transition: 'background .15s' }}
...
export default function CoordinatorDashboard({ user, onLogout, pendingUsers, setPendingUsers, onAssignRole, onRejectUser, notifications, setNotifications }) {
  ...
  const [darkMode, setDarkMode] = useLocalStorage('sitmi_coord_dark_mode', false);

  const pendingCount = (pendingUsers || []).filter(u => !u.role).length;
  ...
  return (
    <div className={`coord-shell${darkMode ? ' coord-dark' : ''}`} style={{ display: 'flex', minHeight: '100vh', background: '#F7F9FC', fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      <SidebarComponent
      ...
      />
      <main className="coord-main" style={{ flex: 1, padding: '28px 32px', overflowY: 'auto' }}>
        {renderView()}
      </main>
    </div>
  );
...
function AIAssistant() {
  ...
  return (
    <div className="coord-ai-shell" style={{ fontFamily: "'Inter', 'Segoe UI', sans-serif", color: '#111827', display: 'flex', gap: 20, height: 'calc(100vh - 56px)' }}>
      {/* Chat */}
      ...
      {/* Sidebar suggestions */}
      <div className="coord-ai-suggestions" style={{ width: 280, display: 'flex', flexDirection: 'column', gap: 16 }}>
