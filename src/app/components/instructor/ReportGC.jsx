...
export default function ReportGC({ onBack }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!mainFile) { showToast('Por favor carga el documento del informe GC', '#ef4444'); return; }
    ...
    try {
      ...
      coordNotifs.unshift({
        ...
        message: `${currentUserName} ha enviado el informe GC de ${month}/${year}`,
        read: false,
        type: 'info',
        date: dateStr,
        color: '#6366f1',
      ...
      showToast('✅ Informe GC enviado para validación');
  ...
  const isImage = mainFile && /\.(jpe?g|png|gif|webp)$/i.test(mainFile.name);
  ...
  return (
    <div style={{ fontFamily: "'Inter','Segoe UI',sans-serif", color: colors.text }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: colors.textMuted }}>Carga tu informe ya diligenciado para que sea validado por supervisión</p>
      </div>
      ...
      <div style={{ background: theme === 'dark' ? '#10233F' : '#EFF6FF', border: `1px solid ${theme === 'dark' ? '#1E3A6B' : '#BFDBFE'}`, borderLeft: '4px solid #2563eb', borderRadius: 12, padding: '14px 18px', marginBottom: 20, display: 'flex', gap: 12 }}>
        ...
        <div style={{ fontSize: 13, color: theme === 'dark' ? '#93C5FD' : '#1d4ed8' }}>
          <div style={{ fontWeight: 700, marginBottom: 6 }}>Este informe ya no se redacta dentro de la plataforma:</div>
          <ul style={{ margin: 0, paddingLeft: 16, lineHeight: 1.9 }}>
            <li>Prepara y diligencia tu informe GC fuera de la plataforma, con las evidencias de las 17 obligaciones</li>
            <li>Carga aquí el documento completo del informe (PDF o Word)</li>
            <li>Supervisión revisará el documento y validará que cumple con todos los parámetros</li>
          </ul>
        </div>
      </div>
      ...
      <div style={{ background: colors.card, borderRadius: 16, border: `1.5px solid ${mainFile ? '#39A900' : colors.border}`, padding: '24px', marginBottom: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Documento del Informe GC</div>
        <div style={{ fontSize: 12, color: colors.textFaint, marginBottom: 16 }}>Sube el informe completo, ya diligenciado, con sus evidencias</div>

        {!mainFile ? (
          <label htmlFor="main-file-gc" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '28px', border: `2px dashed ${colors.borderStrong}`, borderRadius: 12, cursor: 'pointer', background: dropzoneBg, transition: 'all .2s' }}
          ...
          >
            ...
            <span style={{ fontSize: 13, color: colors.textMuted }}>Haz clic para cargar el informe GC</span>
            <span style={{ fontSize: 11, color: colors.textFaint, marginTop: 4 }}>PDF o Word (máx. 20MB)</span>
            <input id="main-file-gc" type="file" style={{ display: 'none' }} onChange={e => handleMainFileChange(e.target.files)} accept=".pdf,.doc,.docx" />
          </label>
        ) : (
          <div>
            {isImage && (
