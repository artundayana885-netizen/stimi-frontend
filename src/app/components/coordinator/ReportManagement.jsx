    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: '#d1d5db', fontWeight: 500, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {report.fileName}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button
            ...
            style={{ width: 28, height: 28, borderRadius: 6, border: 'none', background: currentPage === 1 ? '#374151' : '#4b5563', color: '#fff', cursor: currentPage === 1 ? 'not-allowed' : 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ‹
          </button>
          <span style={{ fontSize: 12, color: '#9ca3af', minWidth: 60, textAlign: 'center' }}>
            Pág. {currentPage} / {report.filePages}
          </span>
          <button
            onClick={() => setCurrentPage(p => Math.min(report.filePages, p + 1))}
            disabled={currentPage === report.filePages}
            style={{ width: 28, height: 28, borderRadius: 6, border: 'none', background: currentPage === report.filePages ? '#374151' : '#4b5563', color: '#fff', cursor: currentPage === report.filePages ? 'not-allowed' : 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ›
          </button>
        </div>
      </div>

      {/* Hoja del documento simulada */}
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ marginTop: 24, paddingTop: 12, borderTop: '1px solid #E8ECF0', display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#9CA3AF' }}>
            ...
            <span>Página {currentPage} de {report.filePages}</span>
