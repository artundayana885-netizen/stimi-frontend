...
// Solo hay informes cargados de enero a julio de 2026 (mes actual: julio 2026)
const MONTHS_WITH_DATA = 7;
...
const AREA_DATA = {
  TIC: {
    ...
    alerts: 1,
    monthly: [
      { gc: 95, gf: 100 }, { gc: 92, gf: 98 }, { gc: 98, gf: 95 }, { gc: 90, gf: 96 },
      { gc: 94, gf: 93 }, { gc: 97, gf: 99 }, { gc: 88, gf: 91 },
    ],
    topInstructors: [
      { name: 'Ana Jiménez', value: 100, initials: 'AJ', color: '#6366f1', bg: '#EEF2FF' },
      { name: 'Miguel Torres', value: 96, initials: 'MT', color: '#6366f1', bg: '#EEF2FF' },
    ],
  },
  PAE: {
    color: '#22c55e',
    ...
    alerts: 0,
    monthly: [
      { gc: 91, gf: 94 }, { gc: 89, gf: 90 }, { gc: 93, gf: 97 }, { gc: 96, gf: 92 },
      { gc: 90, gf: 95 }, { gc: 94, gf: 98 }, { gc: 92, gf: 93 },
    ],
    topInstructors: [
      { name: 'Laura Jiménez', value: 98, initials: 'LJ', color: '#22c55e', bg: '#F0FDF4' },
      { name: 'María González', value: 95, initials: 'MG', color: '#22c55e', bg: '#F0FDF4' },
    ],
  },
  GEA: {
    color: '#f97316',
    ...
    alerts: 1,
    monthly: [
      { gc: 85, gf: 88 }, { gc: 90, gf: 86 }, { gc: 87, gf: 91 }, { gc: 83, gf: 89 },
      { gc: 91, gf: 87 }, { gc: 88, gf: 92 }, { gc: 84, gf: 85 },
    ],
    topInstructors: [
      { name: 'Carlos Rodríguez', value: 92, initials: 'CR', color: '#f97316', bg: '#FFF7ED' },
    ],
  },
  Bioconstrucción: {
    color: '#a855f7',
    ...
    alerts: 0,
    monthly: [
      { gc: 97, gf: 99 }, { gc: 95, gf: 96 }, { gc: 99, gf: 98 }, { gc: 94, gf: 97 },
      { gc: 96, gf: 95 }, { gc: 98, gf: 100 }, { gc: 93, gf: 94 },
    ],
    topInstructors: [
      { name: 'Sofía Ramírez', value: 99, initials: 'SR', color: '#a855f7', bg: '#F5F3FF' },
    ],
  },
};
...
function getMonthlyForArea(areaKey) {
  const areas = areaKey === 'Todas las áreas' ? AREAS : [areaKey];
  const months = [];
  for (let i = 0; i < 12; i++) {
    if (i >= MONTHS_WITH_DATA) { months.push(null); continue; }
    let gcSum = 0, gfSum = 0;
    areas.forEach(a => { gcSum += AREA_DATA[a].monthly[i].gc; gfSum += AREA_DATA[a].monthly[i].gf; });
...
function getStatsForArea(areaKey, monthIndex) {
  const areas = areaKey === 'Todas las áreas' ? AREAS : [areaKey];
  const instructors = areas.reduce((s, a) => s + AREA_DATA[a].instructors, 0);
  const pending = areas.reduce((s, a) => s + AREA_DATA[a].pending, 0);
  const alerts = areas.reduce((s, a) => s + AREA_DATA[a].alerts, 0);
  ...
  if (monthIndex === null) {
    ...
    let total = 0;
    areas.forEach(a => { const m = AREA_DATA[a].monthly[monthIndex]; total += (m.gc + m.gf) / 2; });
    compliance = round(total / areas.length);
  }
...
function getAreaBreakdown() {
  const totalInstructors = AREAS.reduce((s, a) => s + AREA_DATA[a].instructors, 0);
  return AREAS.map(a => ({
    name: a,
    value: round((AREA_DATA[a].instructors / totalInstructors) * 100),
    color: AREA_DATA[a].color,
  }));
}
...
function BarChart({ data }) {
  const max = 100;
  // Ancho de cada grupo de barras: se adapta según cuántos meses hay,
  // pero nunca baja de un mínimo (para que un solo mes no se vea gigante)
  // ni sube de un máximo (para que muchos meses no se amontonen).
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.month}>
              {hasData ? (
                <>
                  <rect x={x} y={chartH - (d.value.gc / max) * chartH} width={barW} height={(d.value.gc / max) * chartH} fill="#22c55e" rx={4} />
                  <rect x={x + barW + 4} y={chartH - (d.value.gf / max) * chartH} width={barW} height={(d.value.gf / max) * chartH} fill="#6366f1" rx={4} />
                </>
              ) : (
                <>
                  <rect x={x} y={chartH - 3} width={barW} height={3} fill="#F0F2F5" rx={2} />
                  <rect x={x + barW + 4} y={chartH - 3} width={barW} height={3} fill="#F0F2F5" rx={2} />
                </>
              )}
              <text x={x + barW} y={chartH + 16} textAnchor="middle" fontSize={9} fill="#9CA3AF">{d.month}</text>
            </g>
          );
        })}
      </svg>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
      <svg width={140} height={140} viewBox="0 0 140 140">
        ...
        <text x={cx} y={cy - 3} textAnchor="middle" fontSize={14} fontWeight={700} fill="#111827">{centerValue}</text>
        <text x={cx} y={cy + 12} textAnchor="middle" fontSize={9} fill="#9CA3AF">{centerLabel}</text>
      </svg>
...
export default function Reports() {
  ...
  const statCards = [
    { icon: '👥', value: yearHasData ? stats.instructors : 0, label: 'Total Instructores', color: '#6366f1', bg: '#EEF2FF', trend: '↑' },
    { icon: '🎯', value: yearHasData && stats.compliance !== null ? `${stats.compliance}%` : '—', label: 'Cumplimiento General', color: '#22c55e', bg: '#F0FDF4', trend: '↑' },
    { icon: '⏰', value: yearHasData ? stats.pending : 0, label: 'Informes Pendientes', color: '#f97316', bg: '#FFF7ED', trend: '↓' },
    { icon: '🚨', value: yearHasData ? stats.alerts : 0, label: 'Con Alertas', color: '#ef4444', bg: '#FEF2F2', trend: '↓' },
  ];
  ...
  return (
    <div style={{ fontFamily: "'Inter', 'Segoe UI', sans-serif", color: '#111827' }}>
      ...
      {/* Filtros */}
      <div className="coord-filters" style={{ background: '#fff', borderRadius: 14, padding: '14px 18px', marginBottom: 20, border: '1px solid #F0F2F5', display: 'flex', gap: 10, alignItems: 'center' }}>
      ...
      {!yearHasData ? (
        <div style={{ background: '#fff', borderRadius: 14, padding: '48px 24px', border: '1px solid #F0F2F5', textAlign: 'center', color: '#6B7280' }}>
          <div style={{ fontSize: 32, marginBottom: 10 }}>🗓️</div>
          <div style={{ fontWeight: 600, fontSize: 15, color: '#111827', marginBottom: 4 }}>Aún no hay informes para {year}</div>
        ...
        <>
          ...
          {/* Top instructors */}
          <div style={{ background: '#fff', borderRadius: 14, padding: 20, border: '1px solid #F0F2F5', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
            <div style={{ fontWeight: 600, fontSize: 15, color: '#111827', marginBottom: 14 }}>
              ...
              {topInstructors.map((inst) => (
                <div key={inst.name} className="coord-instructor-row" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: '#111827', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{inst.name}</div>
